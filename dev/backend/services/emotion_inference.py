...
def predict_emotion(image_file, model_name=None):
    try:
        ...
        logger.debug(f"[DEBUG] Preprocessing image: {image_file}")
        image_array = preprocess_image(image_file)
        logger.debug("[DEBUG] Image preprocessed successfully")
        logger.debug(f"[DEBUG] Image shape: {image_array.shape}")
        if image_array.shape[1:] != (48, 48, 3):
            raise ValueError(
                f"Unexpected image shape: {image_array.shape}, expected (48, 48, 3)")
        logger.debug(f"[DEBUG] Making prediction with model: {model_name}")
        predictions = model.predict(image_array)
        logger.debug("[DEBUG] Prediction completed")
        if predictions.ndim != 2 or predictions.shape[0] != 1:
            raise ValueError(
                f"Unexpected prediction shape: {predictions.shape}")
        logger.debug(f"[DEBUG] Model predictions: {predictions}")
        emotion_index = np.argmax(predictions[0])
        logger.debug(
            f"[DEBUG] Predicted emotion index: {emotion_index}, probabilities: {predictions[0]}")
